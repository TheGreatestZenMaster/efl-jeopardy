class Game < ApplicationRecord
    ...
    QUESTIONS_IN_ROUND = 30
    QUESTIONS = { "L0.1" => {text: "What's this?", image: "banana.png", points: 0},
            "L0.2" => {text: "Good Morning", image: nil, points: 0},
            "L0.3" => {text: "晴れです", image: nil, points: 0},
            "L0.4" => {text: "How are you?", image: nil, points: 0},
            "L0.5" => {text: "I'm _____.", image: "hungry.jpg", points: 0},
            
            "L1.1" => {text: "What's this?", image: "p.png", points: 1},
    ...
    def set_round
        question_set = []
        if questions.limit(30).all? {|x| x.answered == true} 
            (0..4).each {|x| question_set << questions.limit(5).offset((5*x) + 30)}
        else
            (0..5).each {|x| question_set << questions.limit(5).offset(5*x)}
        end
        return question_set
    end
end
